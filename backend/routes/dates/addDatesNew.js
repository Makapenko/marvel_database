...
const fetchMonthData = async (year, month, delayBetweenRequests) => {
  ...
  try {
    ...
    for (const title of comicTitles) {
      const existingComic = await modelComics.findOne({ title })
      if (!existingComic) {
        missingComics.push(title)
      }
    }

    if (missingComics.length > 0) {
      fs.appendFileSync(
        path.join(__dirname, 'missing_comics.txt'),
        missingComics.join('\n') + '\n'
      )
    }
  } catch (error) {
...
async function addDatesNew(req, res) {
